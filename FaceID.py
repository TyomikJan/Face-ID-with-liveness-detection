...
while (cont_loop == True):
                ...
                if len(face_in_feed) == 0:
                    ...
                    for row_num in range(resource_data.nrows):
                        ...
                        if row_value[0] == labels[ID] and resource_names[row_value[0]] == 'no':
                            ...
                            directory_videos = (faces_directory + labels[ID] + '\\videos')
                            fake_directory_videos = (faces_directory + labels[ID] + '\\videos\\' + 'fake.mp4')
                            real_directory_videos = (faces_directory + labels[ID] + '\\videos\\' + 'real.mp4')
                            fake_directory_photos = (faces_directory + labels[ID] + '\\dataset\\' + 'fake')
                            real_directory_photos = (faces_directory + labels[ID] + '\\dataset\\' + 'real')
                            directory_photos = (faces_directory + labels[ID] + '\\dataset')
                            ...
                            question = input('Have you set up the Face ID liveness parameter yet? (y/n) ')
                            if question.lower() == 'нет':
                                ...
                                while video_not_moved:
                                    if (not os.path.exists(fake_directory_videos)) or (not os.path.exists(real_directory_videos)):
                                        question_video = input(f'Record a fake.mp4 and real.mp4 file and move them here: {directory_photos} (enter when finished) ')
                                    else:
                            ...
                            engine.say(f'Access Granted for {labels[ID]}!')
                            engine.runAndWait()
                            cont_loop=False
                else:
    ...
    cv2.imshow('Face ID - Live Output Feed', color_feed)
