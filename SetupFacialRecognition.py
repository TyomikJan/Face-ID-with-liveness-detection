...
def sql_command(name):
    return crsr.execute(f'INSERT INTO authorizations (name) VALUES({name});')
...
# Excel objects:
resource_details = openpyxl.load_workbook(excels_directory + 'ResourceDetails.xlsx')
...
def window(main):
    ...
    main.iconbitmap(images_directory + 'granit.ico')
window(base_window)
        ...
        if set(input_folder_name.get()).issubset(allowed_characters):
            for search_row in resource_data.rows:
                if search_row[0].value == input_folder_name.get():
                    ...
                    global user_name
                    user_name = input_folder_name.get()
                    directory_videos = (faces_directory + user_name + '\\videos')
                    fake_directory_videos = (faces_directory + user_name + '\\videos\\' + 'fake.mp4')
                    real_directory_videos = (faces_directory + user_name + '\\videos\\' + 'real.mp4')
                    ...
                    close_window()
                    os.system(f'mkdir {directory_videos}')
                    ...
                    video_not_moved = True
                    while video_not_moved:
                        if (not os.path.exists(fake_directory_videos)) or (not os.path.exists(real_directory_videos)):
                            question_video = input(f'Record a fake.mp4 and real.mp4 file and move them here: {directory_videos} (enter when finished) ')
                        else:
                            video_not_moved = False

                    os.system(f'cd {liveness_directory} && python -u gather_examples.py --input {fake_directory_videos} --output {fake_directory_photos} --detector face_detector --skip 4')
                    ...
                    sql_command(user_name)
                    connection.commit()
                    connection.close()
                    # DELETING UNNECESSARY PHOTOS and VIDEOS
                    os.system(f'rd /s /q {directory_videos}')
                    #os.system(f'rd /s /q {directory_photos}')
                    #Ending liveness setup
                    break
        else:
...
# Configuration frame:
config_frame = LabelFrame(base_window, text='Enter your credentials')
...
# Password:
Label(config_frame, text='Password :     ').grid(row=1, column=0, sticky=W, padx=7, pady=0)
rdp_password_variable = StringVar()
rdp_password = Entry(config_frame, textvariable=rdp_password_variable, width=33, show='•')
